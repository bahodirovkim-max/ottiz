...
export default function InvoicePage() {
  // Haqiqiy dasturda bu [id] orqali bazadan tortiladi
  const mockInvoice = {
    tenantName: "Sardor Azizov",
    property: "Chilonzor 3-kvartal, 12-uy",
    amount: "400,000 UZS",
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black p-4 sm:p-8 flex flex-col items-center justify-center font-sans tracking-tight">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900/40 p-6 sm:p-10 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-zinc-100 dark:border-zinc-800 transition-all">
        ...
        <div className="text-center mb-10">
           <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-rose-50 dark:bg-rose-500/10 mb-5 relative">
              <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-rose-500 animate-pulse border-2 border-white dark:border-zinc-900"></span>
              <svg className="w-10 h-10 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
           </div>
           <h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2 tracking-tight">To'lov kechikmoqda</h1>
           <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">Ushbu oy uchun to'lovingizni amalga oshiring</p>
        </div>
        ...
        <div className="space-y-5 mb-10">
          <div className="flex justify-between items-center pb-4 border-b border-zinc-100/80 dark:border-zinc-800/80">
            ...
            <span className="text-zinc-900 dark:text-white font-semibold">{mockInvoice.tenantName}</span>
          </div>
          <div className="flex justify-between items-center pb-4 border-b border-zinc-100/80 dark:border-zinc-800/80">
            <span className="text-zinc-500 dark:text-zinc-400 font-medium text-sm">Mulk manzili</span>
            <span className="text-zinc-900 dark:text-white font-semibold text-right max-w-[150px] truncate">{mockInvoice.property}</span>
          </div>
          <div className="flex justify-between items-center pb-4 border-b border-zinc-100/80 dark:border-zinc-800/80">
            <span className="text-zinc-500 dark:text-zinc-400 font-medium text-sm">Eng so'ngi muddat</span>
            <span className="text-rose-600 dark:text-rose-400 font-bold">{mockInvoice.dueDate}</span>
          </div>
          <div className="flex justify-between items-center pt-2">
            <span className="text-zinc-500 dark:text-zinc-400 font-medium text-lg">Jami summa</span>
            <span className="text-black dark:text-white font-bold text-2xl">{mockInvoice.amount}</span>
          </div>
        </div>
